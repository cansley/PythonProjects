...
def extract_names(filename):
  ...
  # +++your code here+++
  file = open(filename)
  searchPattern = r'<td>(\w*)</td>'
  getYearPattern = r'Popularity in (\d\d\d\d)'
  year = 0
  names = []
  for line in file:
      if year == 0:
          match = re.findall(getYearPattern, line)
          if match:
              year = match[0]


      lineItems = re.findall(searchPattern, line)
      if len(lineItems) == 3:
          malename = lineItems[1] + ' ' + str(lineItems[0])
          femalename = lineItems[2] + ' ' + str(lineItems[0])

          if malename not in names:
              names.append(malename)

          if femalename not in names:
